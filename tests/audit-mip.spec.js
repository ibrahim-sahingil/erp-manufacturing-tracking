...
test('MİP: ihtiyaç ile depo stoğunu karşılaştırır, eksiği ve durumu doğru gösterir', async ({ page }) => {
  ...
  try {
    ...
    // (9. tur M4) Tedarik kararı MİP'te verilir — satın alma hesabına düşmesi
    // için pbp'lere PURCHASE kararı yazılır (toplu uç)
    const pbpsKarar = (await api(page, 'GET', '/project-bom-parts/by-project-bom/' + pbomId)).body.data;
    await api(page, 'POST', '/project-bom-parts/decisions',
      { items: pbpsKarar.map(p => ({ id: p.id, decision: 'PURCHASE' })), decided_by: 'TEST' });
